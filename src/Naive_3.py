...
query = """
select Position, HistCTR, IsClick, Price, Title, Params, SearchDate
...
if ind == 1:
  # Get test data
  query_test = """
  select TestID, Position, HistCTR, Price, Params from testSearchStream where ObjectType = 3
  """
  df_test = sql.read_sql(query_test, conn)
  X_test = df_test[['Position', 'HistCTR', 'Price']]

  df_test['Price'] = df_test['Price'].apply(clean_price, 1)
  df_test['Price'] = df_test['Price'].interpolate()


  df_test['params_new'] = (df_test['Params']
                    .apply(lambda x: get_str_list(x), 1)
                    )

  params_X_test = vectorizer.transform(df_test['params_new'])
  params_x_test = svd.transform(params_X_test)
  X_test = np.concatenate((X_test.values, params_X_test.toarray()), 1)
